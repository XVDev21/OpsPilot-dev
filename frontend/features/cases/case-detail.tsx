...
export function CaseDetail({ caseId }: { caseId: string }) {
  ...
  const publish = useMutation({
    mutationFn: (memberId: string | null) =>
      browserApi.publishCase(caseId, memberId),
    onSuccess: refresh,
  });
              ...
              {published ? (
                ...
                <div className="rounded-2xl border border-primary/20 bg-surface-accent p-4">
                  ...
                  <div className="mt-3 grid gap-2">
                    <Button
                      type="button"
                      onClick={() => publish.mutate(publishAssigneeId)}
                      disabled={!publishAssigneeId || publish.isPending}
                    >
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(22rem,0.85fr)]">
        <div className="grid gap-6">
          ...
          >
            ...
            <p className="mt-2 text-xs leading-5 text-foreground-muted">
              Work Items remain available in this PR for backwards
              compatibility. PR 3 moves them into optional Case Tasks and adds
              ...
              {item.workItems.length ? (
                ...
                <div className="rounded-2xl border border-dashed border-border-strong p-5">
                  ...
                  <p className="mt-1 text-xs leading-5 text-foreground-muted">
                    Publishing and assignment do not require a work item. Case
                    Tasks arrive in PR 3.
                  </p>
                </div>
              )}
            </div>
